...
void setup(void)
{
  ...
  EXTI_init(GPIOC, BUTTON_PIN, FALL_EXTI, 2);
  ...
  GPIO_pupdr(GPIOC, BUTTON_PIN, PD);	
	...
	GPIO_pupdr(GPIOB, 8, PD);
	
	//----------------Light intensity sensor(Curtain)--------------//
	GPIO_init(GPIOB, 4, INPUT);
	EXTI_init(GPIOB,4,BOTH_EXTI,2);
	GPIO_pupdr(GPIOB, 4, PD);
	...
	// PWM configuration -----------------------------------------------------------------	
	PWM_t trig;											// PWM1 for trig
	PWM_init(&trig,GPIOA,1);			 	// PWM init as PA5: Ultrasonic trig pulse TIM2_CH2
	PWM_period_us(&trig,100000);    	// PWM of 50ms period. Use period_us()
	PWM_pulsewidth_us(&trig,10);   	// PWM pulse width of 10us
	
	// Input Capture configuration -------------------------------------------------------
	IC_t echo;											// Input Capture for echo
	ICAP_init(&echo,GPIOB,9);    	// ICAP init as PB6 as input caputre
 	ICAP_counter_us(&echo, 10);   	// ICAP counter step time as 10us
	ICAP_setup(&echo, 3, RISE_TIM);   	// TIM4_CH1 as IC1 , rising edge detect
	ICAP_setup(&echo, 4, FALL_TIM);   	// TIM4_CH1 as IC2 , falling edge detect
