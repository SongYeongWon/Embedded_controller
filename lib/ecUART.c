...
//1. GPIO Pin for TX and RX	
	...
	if (USARTx==USART6){ 
		// USART_TX GPIO AFR
		if(pinTX<8) GPIO_TX->AFR[0] |= 8UL<<(4*pinTX);  		 
		else GPIO_TX->AFR[1] |= 8UL<<(4*(pinTX%8));
		// USART_RX GPIO AFR
		if(pinRX<8) GPIO_RX->AFR[0] |= 8UL<<(4*pinRX);  	 	 
		else GPIO_RX->AFR[1] |= 8UL<<(4*(pinRX%8));  			
	}
	else{	//USART1,USART2
		// USART_TX GPIO AFR
		if(pinTX<8) GPIO_TX->AFR[0] |= 7UL<<(4*pinTX);  	 	 
		else GPIO_TX->AFR[1] |= 7UL<<(4*(pinTX%8));  			 
		// USART_RX GPIO AFR
		if(pinRX<8) GPIO_RX->AFR[0] |= 7UL<<(4*pinRX);  	
		else GPIO_RX->AFR[1] |= 7UL<<(4*(pinRX%8));  			
	}
	// No pull up, No pull down 
  GPIO_pupdr(GPIO_TX, pinTX, NOPUPD);
	GPIO_pupdr(GPIO_RX, pinRX, NOPUPD);
...
//2. USARTx (x=2,1,6) configuration	
	...
	USARTx->CR1 &= ~(1UL<<10);   	// No parrity bit
	USARTx->CR1 &= ~(1UL<<12);    // M: 0 = 8 data bits, 1 start bit    
	USARTx->CR1 &= ~(1UL<<15);  	// 0 = oversampling by 16 (to reduce RF noise)	 
	// Configure Stop bit
	USARTx->CR2 &= ~USART_CR2_STOP;  	// 1 stop bit																 
...
// 3. Read USARTx Data (Interrupt)	
	...
	if (USARTx==USART1){
		...
		NVIC_SetPriority(USART2_IRQn, 6);      		// Set Priority to 1
		NVIC_EnableIRQ(USART2_IRQn);             	// Enable interrupt of USART2 peripheral
	}
	else {																			// if(USARTx==USART6)
		NVIC_SetPriority(USART6_IRQn, 5);      		// Set Priority to 1
