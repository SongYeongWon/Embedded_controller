...
* @brief   Embedded Controller:  EC_HAL_PWM 
* 
******************************************************************************
*/
#include "ecPWM.h"
...
// 1. Initialize GPIO port and pin as AF
		...
		GPIO_init(port, pin, ALTERNATE );  // AF=2
		GPIO_ospeed(port, pin,FAST_SPEED);  // speed VHIGH=3
		GPIO_otype(port, pin, OUTPUT_PP);
		GPIO_pupdr(port, pin, NOPUPD);

// 2. Configure GPIO AFR by Pin num.				
	//  AFR[0] for pin: 0~7,     AFR[1] for pin 8~15
	//  AFR=1 for TIM1,TIM2	AFR=2 for TIM3 etc
	//GPIOA
	GPIOA->AFR[0] &= ~(15UL << (4*pin));
	GPIOA->AFR[1] &= ~(15UL << (4*pin%8));

	//GPIOB
	GPIOB->AFR[0] &= ~(15UL << (4*pin));
	GPIOB->AFR[1] &= ~(15UL << (4*pin%8));
	...
	if(8<=pin && pin<=15){
	GPIOA->AFR[1] |= 1 << (4*pin%8);
	GPIOB->AFR[1] |= 1 << (4*pin%8);
		}
	}
	//GPIOA->AFR[pin>>3] |= 1<<(4*pin%8); shift right is divide binary to 2 nth. pin/8
	if(TIMx == TIM3 || TIMx == TIM4 || TIMx == TIM5){ 
	if(0<=pin && pin<8){ 
	GPIOA->AFR[0] |= 2UL << (4*pin);
	GPIOB->AFR[0] |= 2UL << (4*pin);
	}
	if(8<=pin && pin<=15){
	GPIOA->AFR[1] |= 2UL << (4*pin%8);
	GPIOB->AFR[1] |= 2UL << (4*pin%8);
		}
	}
	if(TIMx == TIM9 || TIMx == TIM11){ 
	if(0<=pin && pin<8){ 
	GPIOA->AFR[0] |= 3UL << (4*pin);
	GPIOB->AFR[0] |= 3UL << (4*pin);
	}
	if(8<=pin && pin<=15){
	GPIOA->AFR[1] |= 3UL << (4*pin%8);
	GPIOB->AFR[1] |= 3UL << (4*pin%8);
		}
	}
			
// 3. Initialize Timer 
		//TIM_init(TIMx, 1);	// with default msec=1 value.		
		TIM_usec_init(TIMx,1);
// 3-2. Direction of Counter
		TIMx->CR1 &= 0<<4;    // Counting direction: 0 = up-counting, 1 = down-counting
	
// 4. Configure Timer Output mode as PWM
	uint32_t ccVal=TIMx->ARR/2;  // default value  CC=ARR/2
	if(CHn == 1){
		TIMx->CCMR1 &= ~TIM_CCMR1_OC1M;                     // Clear ouput compare mode bits for channel 1
		TIMx->CCMR1 |= 6 << 4; 															// OC1M = 110 for PWM Mode 1 output on ch1. #define TIM_CCMR1_OC1M_1          (0x2UL << TIM_CCMR1_OC1M_Pos)
		...
		TIMx->CCMR1 |= 6 << 12; 															// OC1M = 110 for PWM Mode 1 output on ch2
		...
		TIMx->CCMR2 |= 6 << 4; 															// OC1M = 110 for PWM Mode 1 output on ch3
	...
	TIMx->CCMR2 &= ~TIM_CCMR2_OC4M;                  		  // Clear ouput compare mode bits for channel 4
		TIMx->CCMR2 |= 6 << 12; 															// OC1M = 110 for PWM Mode 1 output on ch4
...
void PWM_duty(PWM_t *pwm, float duty) {                 //  duty=0 to 1	
	TIM_TypeDef *TIMx = pwm->timer;
  uint16_t ARR_VAL = TIMx->ARR ;
	float ccval = (ARR_VAL +1)*duty;    								// CCR = (ARR+1)*dutyRatio -> duty = CCR/(ARR +1)          
		int CHn = pwm->ch;
...
void PWM_pinmap(PWM_t *PWM_pin){
   ...
   if(port == GPIOA) {
      switch(pin){
         case 0 : PWM_pin->timer = TIM2; PWM_pin->ch = 1; break;		//TIM5_channel 1
         case 1 : PWM_pin->timer = TIM2; PWM_pin->ch = 2; break;		
				 case 2 : PWM_pin->timer = TIM5; PWM_pin->ch = 3; break;		//TIM5_chnnnel 2
         case 5 : PWM_pin->timer = TIM2; PWM_pin->ch = 1; break;		//TIM5_chnnnel 3
         case 6 : PWM_pin->timer = TIM3; PWM_pin->ch = 1; break;		//TIM5_chnnnel 4
      ...
      switch(pin){
         ...
         case 9 : PWM_pin->timer = TIM4; PWM_pin->ch = 4; break;
